...
INPUT_DIR = BASE_DIR / "Door TestCases"  / "BinPacking" / "Inputs" / "Json"
BASELINE_DIR = BASE_DIR / "Door TestCases" / "BinPacking" / "Baselines" / "JsonSingleFileOutputs"
...
TEST_FILES = sorted([p for p in INPUT_DIR.glob("*.json")])
...
def test_binpacking_json_against_baseline(input_file: Path):
    expected_file = BASELINE_DIR / (input_file.stem + OUTPUT_SUFFIX)

    assert expected_file.exists(), (
        f"Missing binpacking baseline output for {input_file.name}"
    )

...
def export_dxf_to_svg_headless(
    dxf_source: Union[str, os.PathLike, Drawing],
    svg_path: Union[str, os.PathLike],
    margin_mm: float = 8.0,
):
    """
    DXF ➜ SVG Export using Matplotlib backend.
    """
    # 🔹 Load DXF
    ...
    # 🔹 Render Setup (matplotlib)
    import matplotlib.pyplot as plt
    fig, ax = plt.subplots(figsize=(210/25.4, 297/25.4), dpi=96)  # A4 in inches
    ...
    # ✨ Save SVG using matplotlib
    fig.savefig(svg_path, format="svg", bbox_inches='tight')
    plt.close(fig)

    print(f"🎯 Exported DXF ➜ SVG successfully: {svg_path}")

# 🚀 Example (Run directly)
if __name__ == "__main__":
    input_dxf_file = os.path.join("Door TestCases", "DoorGeometry", "Baselines", "Dxf", "SingleNormal.dxf")
    output_svg_file = os.path.splitext(input_dxf_file)[0] + ".svg"

    if os.path.exists(input_dxf_file):
        print(f"Processing: {input_dxf_file}...")
        export_dxf_to_svg_headless(input_dxf_file, output_svg_file, margin_mm=10.0)
    else:
